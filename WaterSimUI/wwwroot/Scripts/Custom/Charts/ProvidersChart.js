...
function drawProvidersChart($jsonObj, controlID, subcontrols, fldLabName, fldValue, providerName, strtYr, endYr, index, Min, Max, Units, seriescolors, chartTypeCode, currentChartObject) {
   ...
   // Ymax = 200;
    ...
    // Demand TAB DAS --------------------------------------------------------------------
    // Create the chart
    if (currentChartObject == undefined || currentChartObject == null) {
        console.log("currentChartObject undefined");
        var $myChart = {
        ...
        //SXI1211 07/03/2017
        //Description: Check for an existing chart object then create if it already does not exist
    else {
        console.log("currentChartObject defined");
        for (count = 0; count < chartsDataArray.length; count++) {
            currentChartObject.series[count].setData(chartsDataArray[count], false);
        }
        currentChartObject.redraw();
    return currentChartObject;
    }
